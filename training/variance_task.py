...
class VarianceDataset(BaseDataset):
    def __init__(self, config, prefix, preload=False):
        ...
        need_energy = self.config['predict_energy']
        need_breathiness = self.config['predict_breathiness']
        need_voicing = self.config['predict_voicing']
        need_tension = self.config['predict_tension']
...
class VarianceTask(BaseTask):
    def __init__(self, config):
        ...
        self.diffusion_type = self.config['diffusion_type']

        self.use_spk_id = self.config['use_spk_id']

        self.predict_dur = self.config['predict_dur']
        if self.predict_dur:
            self.lambda_dur_loss = self.config['lambda_dur_loss']

        self.predict_pitch = self.config['predict_pitch']
        if self.predict_pitch:
            self.lambda_pitch_loss = self.config['lambda_pitch_loss']
        ...
        self.lambda_var_loss = self.config['lambda_var_loss']
        super()._finish_init()
