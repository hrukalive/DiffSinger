...
class AcousticDataset(BaseDataset):
    def __init__(self, config, prefix, preload=False):
        ...
        self.need_key_shift = self.config['use_key_shift_embed']
        self.need_speed = self.config['use_speed_embed']
        self.need_spk_id = self.config['use_spk_id']
...
class AcousticTask(BaseTask):
    def __init__(self, config):
        ...
        self.use_shallow_diffusion = self.config['use_shallow_diffusion']
        if self.use_shallow_diffusion:
            self.shallow_args = self.config['shallow_diffusion_args']
            self.train_aux_decoder = self.shallow_args['train_aux_decoder']
            self.train_diffusion = self.shallow_args['train_diffusion']

        self.use_vocoder = self.config['infer'] or self.config['val_with_vocoder']
        if self.use_vocoder:
            self.vocoder: BaseVocoder = get_vocoder_cls(self.config)(self.config)
