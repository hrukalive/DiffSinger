...
#         super().__init__()
#         self.satt = Attention(dim, heads=heads, dim_head=dim_head, cdim=dim)
...
#         self.nm1= nn.LayerNorm(dim)
#         self.nm2 = nn.LayerNorm(dim)
#
#         # self.nm3 = nn.GroupNorm(32, cdim)
#
#     def forward(self, x, c):
#         torch
#         nx=self.nm(x.transpose(1,2)).transpose(1,2)
#         x = self.satt(nx,nx) + x
#         x = self.catt(self.nm1(x.transpose(1,2)).transpose(1,2), c) + x
#         x = self.mlp(self.nm2(x.transpose(1,2)).transpose(1,2)) + x
#         return x
...
class unetb(nn.Module):
    def __init__(self, downs=[2, 2, 2], dim=[256, 384, 512], latentdim=512, indim=128, cdim=256):
        ...
        # self.mlp = (
        #     nn.Sequential(nn.GroupNorm(32, 512),nn.SiLU(), nn.Linear(512, 256))
        #     # if time_emb_dim is not None
        #     # else None
        # )
        ...
        for idx, i in enumerate(downs):
            self.dlres.append(nn.ModuleList([ResnetBlock(dim[idx], dim[idx], time_emb_dim=512, groups=32),
                                             ResnetBlock(dim[idx], dim[idx], time_emb_dim=512, groups=32)]))
            h = dim[idx] // 32
            self.dlatt.append(
                nn.ModuleList([attLp(dim[idx], heads=h, cdim=cdim), attLp(dim[idx], heads=h, cdim=cdim)]))
            ...
            # self.dldd4.append(nn.ModuleList([downblock(igx , cdim, dims[idx ]) ,downblock(igx , cdim, dims[idx ]) ]) if igx!=1 else nn.ModuleList([nn.Conv1d(cdim,dim[0],kernel_size=1),nn.Conv1d(cdim,dim[0],kernel_size=1)]))
        ...
        for idx, i in enumerate(ups):
            self.upres.append(nn.ModuleList([ResnetBlock(upsd[idx] * 2, upsd[idx], time_emb_dim=512, groups=32),
                                             ResnetBlock(upsd[idx] * 2, upsd[idx], time_emb_dim=512, groups=32)]))
            h = dim[idx] // 32
            self.upatt.append(
                nn.ModuleList([attLp(upsd[idx], heads=h, cdim=cdim), attLp(upsd[idx], heads=h, cdim=cdim)]))
