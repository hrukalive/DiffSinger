...
class unetb(nn.Module):
    def __init__(self, downs=[2, 2, 2], dim=[256, 384, 512], latentdim=512, indim=128, cdim=256):
        ...
        for idx, i in enumerate(downs):
            ...
            h = dim[idx] // 32
            self.dlatt.append(
                nn.ModuleList([attLIN(dim[idx], heads=h, cdim=cdim), attLIN(dim[idx], heads=h, cdim=cdim)]))
        ...
        for idx, i in enumerate(ups):
            ...
            h = dim[idx] // 32
            self.upatt.append(
                nn.ModuleList([attLIN(upsd[idx], heads=h, cdim=cdim), attLIN(upsd[idx], heads=h, cdim=cdim)]))
