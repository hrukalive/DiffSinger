...
class ResnetBlock(nn.Module):
    ...
    def forward(self, x, time_emb=None, cs=None):
        scale_shift = None
...
class unetb(nn.Module):
    ...
    def forward(self, x, time_emb, cs):
        ...
        for res, att, don, csd in zip(self.dlres, self.dlatt, self.dldd, self.dldd4):
            ...
            fff.append(cccc)
            x = don(x)
        x = self.mres2(self.matt(self.mres1(x, time_emb,self.dldd3[0](cs)), csa), time_emb,self.dldd3[1](cs))
        fff.reverse()
        ...
        for res, att, up, fautres,csud in zip(self.upres, self.upatt, self.updd, fff,self.dldd2):
            ...
            for idx, ii in enumerate(res):
                ...
                x = ii(x, time_emb,csud[idx](cs))
                x = att[idx](x, csa)

        x = torch.cat([x, res11], dim=1)
        for ii in self.outres:
            x = ii(x, time_emb,self.csds(cs))
        x = self.outatt(x, csa)
